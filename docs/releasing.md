...
requests and publish bottles to GitHub Packages when a maintainer applies the
`pr-pull` label after the checks pass. If `main` is protected, configure the
ruleset to let the generated publisher workflow push its bottle commit;
otherwise the final `brew pr-pull` push will be blocked.
...
4. Apply the `pr-pull` label. The tap's generated workflow runs `brew pr-pull`,
   publishes the bottles, pushes the bottle metadata commit, and removes the
   release branch.
...
On macOS select the downloaded filename before checking it, for example:

```bash
grep 'portless_1.2.3_darwin_arm64.tar.gz' checksums.txt | shasum -a 256 -c -
```

The GitHub CLI attestation command is the same on both platforms.
