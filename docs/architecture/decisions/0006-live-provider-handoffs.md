...
Changing an environment source path remains stopped-only. Source rediscovery can alter multiple service definitions and dependency edges, so it is not equivalent to selecting a different provider for one already-declared service.
