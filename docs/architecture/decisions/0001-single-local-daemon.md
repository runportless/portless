...
This catches stale development daemons even when their coarse API version has not changed. It also prevents a healthy process listening on the recorded port, a reused PID, or an edited record from being treated as the daemon. Identity and shutdown routes require CLI bearer authentication and are not served on application hosts or available to browser sessions.

An authenticated older build is replaced automatically only when the daemon reports no active environments. Explicit `portless daemon stop` and `portless daemon restart` use the same authenticated shutdown and have the same default guard. `--force` may interrupt active environments and can replace a legacy daemon after additionally checking process ownership and command arguments. Raw PID signaling is never the normal control path.
