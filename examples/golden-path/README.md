...
- `gateway`: a NestJS-shaped Node service with `ORDERS_URL` and Redis-compatible configuration.
...
- `gateway → orders`, `gateway → redis`, `orders → postgres`, and `orders → redis` edges.
...
After startup, request `http://gateway.golden-path.localhost:7331/checkout`. The external request and internal `gateway → orders` call appear as separate traffic events. Add a latency or 503 fault to the internal edge and repeat the request.
