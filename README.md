...
Local application processes are children of small authenticated Portless supervisors rather than the daemon itself. On daemon restart, Portless reattaches to those supervisors, adopts managed containers by installation/environment/service/generation labels, restores dependency proxies on their original injected ports, then republishes ingress. Services remain in `recovering` until that verification completes. A known process exit remains `exited` or `failed`. After a machine reboot or equivalent runtime loss, a stale non-terminal supervisor record becomes `stopped` only when its durable identity matches and both its supervisor PID and full application process group are absent. A fully owned stopped container, or an expected container proven absent from the reachable persisted engine, also becomes `stopped`; the next `portless up` recreates those services at a new generation while preserving managed volumes. Any live, mismatched, incomplete, ambiguous, or uninspectable evidence remains `unknown`, so Portless never launches a duplicate or signals an unverified process.
