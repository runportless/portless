...
test('creates and hot-binds a mock profile without restarting peer services', async ({ page }) => {
  ...
  await expect(profileRow).toContainText('bound')
  await expect(profileRow.getByRole('button', { name: 'Disable sold-out' })).toBeVisible()
  ...
  await profileRow.getByRole('button', { name: 'Disable sold-out' }).click()
  await expect(profileRow.getByRole('button', { name: 'Enable sold-out' })).toBeVisible({ timeout: 30_000 })
