...
test('creates and hot-binds a mock profile without restarting peer services', async ({ page }) => {
  ...
  await page.getByRole('button', { name: 'CREATE MOCK', exact: true }).click()
  await expect(page).toHaveURL(/tab=mocks&workspace=create/)
  const mockWorkspace = page.getByRole('region', { name: 'Create mock' })
  await expect(mockWorkspace).toBeVisible()
  await expect(page.getByRole('dialog', { name: /Create mock/i })).toHaveCount(0)
  const profileName = mockWorkspace.getByLabel('NAME')
  await expect(profileName).toBeFocused()
  await profileName.fill('sold-out')
  await mockWorkspace.getByLabel('SERVICE').selectOption('inventory')
  await mockWorkspace.getByLabel('DESCRIPTION').fill('Inventory has no available stock')
  await mockWorkspace.getByLabel('PATH', { exact: true }).fill('/inventory/{sku}')
  await mockWorkspace.getByLabel('RESPONSE STATUS').selectOption('200')
  await mockWorkspace.getByLabel('RESPONSE BODY').fill('{"available":false,"reason":"mocked sold out"}')
  await mockWorkspace.getByText('ADVANCED MATCHING & RESPONSE').click()
  await mockWorkspace.getByLabel('ROUTE NAME').fill('lookup')
  const routeEditor = mockWorkspace.getByRole('region', { name: 'Route editor' })
  await mockWorkspace.getByRole('button', { name: 'Maximize route editor' }).click()
  await expect(routeEditor).toHaveClass(/is-maximized/)
  await expect(mockWorkspace.getByRole('button', { name: 'Restore route editor' })).toBeVisible()
  await page.keyboard.press('Escape')
  await expect(routeEditor).not.toHaveClass(/is-maximized/)
  await mockWorkspace.getByRole('button', { name: 'PREVIEW', exact: true }).click()
  await mockWorkspace.getByRole('button', { name: 'RUN', exact: true }).click()
  const draftPreview = mockWorkspace.locator('.mock-draft-preview-result')
  await expect(draftPreview).toContainText('MATCHED · lookup')
  await expect(draftPreview).toContainText('200')
  await mockWorkspace.getByLabel('Enable for inventory after creation').check()
  const createAndEnable = mockWorkspace.getByRole('button', { name: 'CREATE & ENABLE', exact: true })
  await expect(createAndEnable).toBeEnabled()
  await createAndEnable.click()
  await expect(mockWorkspace).toHaveCount(0, { timeout: 30_000 })
  ...
  await expect(page).toHaveURL(/tab=mocks&workspace=route&profile=sold-out/)
  const createRouteWorkspace = page.getByRole('region', { name: 'Create Route' })
  await expect(createRouteWorkspace).toBeVisible()
  await expect(page.getByRole('dialog', { name: 'Create Route' })).toHaveCount(0)
  await expect(createRouteWorkspace.getByLabel('PATH', { exact: true })).toBeFocused()
  await createRouteWorkspace.getByRole('button', { name: 'BACK TO SOLD-OUT' }).click()
  await expect(createRouteWorkspace).toHaveCount(0)
  await expect(mockDrawer).toBeVisible()
  ...
  const editRoute = routeRow.getByRole('button', { name: 'EDIT', exact: true })
  await expect(editRoute).toBeVisible()
  await editRoute.click()
  await expect(page).toHaveURL(/tab=mocks&workspace=route&profile=sold-out&route=lookup/)
  const editRouteWorkspace = page.getByRole('region', { name: 'Edit Route' })
  await expect(editRouteWorkspace).toBeVisible()
  await expect(editRouteWorkspace.getByLabel('PATH', { exact: true })).toHaveValue('/inventory/{sku}')
  await expect(editRouteWorkspace.getByLabel('ROUTE NAME')).toBeDisabled()
  await editRouteWorkspace.getByRole('button', { name: 'BACK TO SOLD-OUT' }).click()
  await expect(mockDrawer).toBeVisible()
