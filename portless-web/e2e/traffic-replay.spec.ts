...
test('opens without sending, edits one live HTTP request, and compares the real response losslessly', async ({ page }, testInfo) => {
  ...
  const differences = editor.getByRole('button', { name: 'Differences found', exact: true })
  const bodyChanges = editor.getByRole('table', { name: 'body changes' })
  await expect(differences).toHaveAttribute('aria-expanded', 'true')
  await differences.click()
  await expect(differences).toHaveAttribute('aria-expanded', 'false')
  await expect(bodyChanges).toBeHidden()
  await expectResponseFillsHeight(editor)
  await differences.press('Enter')
  await expect(differences).toHaveAttribute('aria-expanded', 'true')
  await expect(bodyChanges).toContainText('/body')
  await differences.press('Space')
  await expect(differences).toHaveAttribute('aria-expanded', 'false')
  await expect(bodyChanges).toBeHidden()
  await expect(differences).toBeFocused()
  for (const [size, colorScheme] of [[viewport, 'dark'], [viewport, 'light'], [{ width: 580, height: 900 }, 'light']] as const) {
    await page.setViewportSize(size)
    await page.emulateMedia({ colorScheme })
    await differences.scrollIntoViewIfNeeded()
    await expect(differences).toBeInViewport()
    for (const response of await editor.locator('.replay-response').all()) {
      expect(await response.locator('.replay-response__body').evaluate((element) => element.clientHeight)).toBeGreaterThan(30)
    }
    expect(await editor.evaluate((element) => element.scrollWidth <= element.clientWidth)).toBe(true)
    await page.screenshot({ path: testInfo.outputPath(`replay-diff-collapsed-${size.width}-${colorScheme}.png`), animations: 'disabled' })
  }
  await page.setViewportSize(viewport)
  await page.emulateMedia({ colorScheme: 'dark' })
  await editor.getByRole('tab', { name: 'Original', exact: true }).click()
  ...
  await editor.getByRole('region', { name: 'Original raw', exact: true }).getByRole('tab', { name: 'Body', exact: true }).click()
  await expect(differences).toHaveAttribute('aria-expanded', 'false')
  await expect(bodyChanges).toBeHidden()
  await editor.getByRole('region', { name: 'Original body', exact: true }).getByRole('tab', { name: 'Raw', exact: true }).click()
  expect(runRequests).toBe(1)
  await expectDiffSummary(editor, true)
  await expectResponseFillsHeight(editor)
