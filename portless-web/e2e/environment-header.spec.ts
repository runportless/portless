  ...
  try {
    ...
    for (const theme of ['light', 'dark']) {
      ...
      for (const width of [1440, 1024, 768, 760, 390, 320]) {
        ...
        await expect(header.getByRole('button', { name: 'START ALL', exact: true })).toBeVisible()
    ...
    await header.getByRole('button', { name: `Environment actions for ${state.project}/${name}` }).click()
    const menu = page.getByRole('menu', { name: `${state.project}/${name} actions` })
    await expect(menu).toContainText(`Created by cloning ${state.project}/${state.environment}; changes are independent.`)
...
test('shares a single activity subscription across views and discards a late previous-environment snapshot', async ({ page }) => {
  ...
  try {
    ...
    await palette.getByPlaceholder('jump to a project or environment').fill(`${state.debugProject}/${state.environment}`)
...
test('shares the pending lifecycle action between the header and command palette', async ({ page }) => {
  ...
  try {
    const stop = environmentHeader(page).getByRole('button', { name: 'STOP ALL', exact: true })
    await stop.evaluate((button: HTMLButtonElement) => { button.click(); button.click() })
    await expect(environmentHeader(page).getByRole('button', { name: 'STOPPING…', exact: true })).toBeDisabled()
    ...
    await page.keyboard.press('Control+K')
    const palette = page.getByRole('dialog', { name: 'Command palette' })
    await palette.getByPlaceholder('jump to a project or environment').fill('environment')
    await expect(palette.getByRole('button', { name: /^(Start|Stop) environment/ })).toHaveCount(0)
    await page.keyboard.press('Escape')
    release()
    await expect(environmentHeader(page).getByRole('button', { name: 'START ALL', exact: true })).toBeEnabled({ timeout: 30_000 })
    expect(requests).toBe(1)
    await page.keyboard.press('Control+K')
    await palette.getByPlaceholder('jump to a project or environment').fill('Start environment')
    await palette.getByRole('button', { name: /Start environment/ }).click()
    await expect(environmentHeader(page).getByRole('button', { name: 'STOP ALL', exact: true })).toBeEnabled({ timeout: 30_000 })
    await expect(environmentHeader(page).getByRole('link', { name: /health: healthy/ })).toBeVisible()
    ...
    await environmentHeader(page).getByRole('button', { name: 'STOP ALL', exact: true }).click()
