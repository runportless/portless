...
test('paginates traces and exchanges at 25 rows', async ({ page }) => {
  ...
  const summary = trace.getByRole('group', { name: 'Trace summary', exact: true })
  await expect(summary.getByRole('heading')).toContainText(`GET ${marker}`)
  await expect(summary.getByTitle('Environment', { exact: true })).toHaveText('ui-e2e/local')
  await expect(summary.getByTitle('Root service route', { exact: true })).toHaveText('external → checkout')
  await expect(summary.getByTitle('Result', { exact: true })).toHaveText('404')
  await expect(summary.getByTitle('Result', { exact: true })).toHaveClass('warning-text')
  await expect(summary.getByTitle('Duration', { exact: true })).toHaveText(/\d+(\.\d+)?(ms|s)/)
  await expect(summary.locator('time')).toHaveText(/\d{1,2}:\d{2}:\d{2}\.\d{3}/)
