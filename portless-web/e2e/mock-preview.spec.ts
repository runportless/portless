...
test('switches Routes and Preview while retaining the selected route, configuration, draft, and result', async ({ page }) => {
  ...
  try {
    ...
    await expect(editor.locator('.mock-route-configuration-name')).toHaveText('lookup')
...
test('keeps Routes and Preview usable in both themes and narrow workspaces', async ({ page }, testInfo) => {
  const fixture = await createScenario('layouts', [route('lookup', '/inventory/{sku}', '{"available":false,"quantity":0}')])
  try {
    await authenticate(page, fixture.path + '&route=lookup')
    const editor = page.getByRole('region', { name: 'Edit Route', exact: true })
...
test('previews saved route overlays, precedence, no matches, disabled routes, and outdated results', async ({ page }, testInfo) => {
  ...
  try {
    ...
    const disabledBadge = editor.locator('.mock-route-configuration-bar .mock-route-disabled-state')
    ...
    for (const width of [760, 540]) {
      await page.setViewportSize({ width, height: 820 })
      await expect(disabledBadge).toBeInViewport()
      await expect(editor.locator('.mock-route-configuration-name')).toBeInViewport()
      await expect.poll(() => page.evaluate(() => document.documentElement.scrollWidth)).toBeLessThanOrEqual(width)
