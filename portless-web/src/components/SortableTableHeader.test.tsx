...
describe('SortableTableHeader', () => {
  it('renders paired direction triangles and identifies the active sort', () => {
    const html = renderToStaticMarkup(<table><thead><tr>
      <SortableTableHeader label="State" sortKey="state" sort={{ key: 'state', direction: 'asc' }} itemCount={2} onSort={() => undefined} />
      <SortableTableHeader label="Name" sortKey="name" sort={{ key: 'state', direction: 'asc' }} itemCount={2} onSort={() => undefined} />
    </tr></thead></table>)
  ...
  it('marks a descending grid sort while keeping one active column', () => {
    const html = renderToStaticMarkup(<div role="row">
      <SortableGridHeader label="State" sortKey="state" sort={{ key: 'name', direction: 'desc' }} itemCount={2} onSort={() => undefined} />
      <SortableGridHeader label="Name" sortKey="name" sort={{ key: 'name', direction: 'desc' }} itemCount={2} onSort={() => undefined} />
    </div>)
    ...
    expect(html).not.toContain('is-default-sort')
  ...
  it('omits the sort control when the table has fewer than two items', () => {
    const html = renderToStaticMarkup(<table><thead><tr>
      <SortableTableHeader label="Name" sortKey="name" sort={{ key: 'name', direction: 'asc' }} itemCount={1} onSort={() => undefined} />
    </tr></thead></table>)
