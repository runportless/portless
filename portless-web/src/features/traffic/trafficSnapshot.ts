...
export async function loadTrafficSnapshot(environment: Pick<Environment, 'project' | 'name'>, edgeFilter: string): Promise<TrafficSnapshot> {
  ...
  const exchangeResult = await api<{ exchanges: TrafficExchange[] }>(environmentPath(environment, '/traffic/exchanges?protocol=all&limit=1000'))
  ...
  const traceResult = await api<{ traces: TrafficTrace[] }>(environmentPath(environment, `/traffic/traces?background=include&limit=1000${edge}`))
