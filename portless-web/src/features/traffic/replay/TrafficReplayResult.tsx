...
export function TrafficReplayResult({ workspace, outdated }: { workspace: TrafficReplayWorkspace; outdated: boolean }) {
  ...
  const replayed = result?.exchange
  const trafficURL = replayed ? `/environments/${encodeURIComponent(replayed.project)}/${encodeURIComponent(replayed.environment)}?${new URLSearchParams({ tab: 'traffic', edge: `${replayed.source}:${replayed.target}`, protocol: 'http' })}` : ''
  return <section className="replay-result" aria-label="Replay response comparison">
    <div className="replay-section-heading"><h3>RESPONSE COMPARISON</h3>{result && <span role="status">{outdated ? 'From previous request' : `Run ${result.runNumber}`}</span>}</div>
    ...
    <div className="replay-result__content" id={panelID} role="tabpanel" aria-label={view === 'diff' ? 'Response diff' : `${view} response`}>
      ...
      {replayed && <p className="replay-destination-link"><a href={trafficURL}>Open destination Traffic</a><span>{replayed.project}/{replayed.environment} · request #{replayed.sequence}</span></p>}
      ...
      <ReplayTabs label="Replay response representation" value={representation} options={[{ value: 'body', label: 'Body' }, { value: 'headers', label: 'Headers' }, { value: 'raw', label: 'Raw' }]} onChange={setRepresentation} panelID={representationID} />
      <div id={representationID} role="tabpanel" tabIndex={0} aria-label={`${view} response ${representation}`}>
        {view === 'diff' ? result ? <ReplayDifference section={representation === 'raw' ? undefined : result.comparison[representation]} original={workspace.baseline} replayed={result.exchange} representation={representation} /> : <p className="replay-empty">Send replay to compare the captured response with a new response.</p>
          : selected ? <ReplayResponse exchange={selected} representation={representation} label={view === 'original' ? 'Original' : 'Replayed'} /> : <p className="replay-empty">No replay response available.</p>}
      </div>
    </div>
  </section>
}
...
function ReplayResponse({ exchange, representation, label }: { exchange: TrafficExchange; representation: TrafficPayloadView; label: string }) {
  ...
  return <section className="replay-response" aria-label={`${label} ${representation}`}>
    <div className="replay-response__heading"><strong>{label}</strong><span>{exchange.status || 'No HTTP response'} · {exchange.durationMs} ms</span><button type="button" className="traffic-copy-button" aria-label={`Copy ${label.toLowerCase()} response ${representation}`} onClick={() => void navigator.clipboard.writeText(text).catch(() => undefined)}>COPY</button></div>
    <div className="replay-response__body" tabIndex={0}>
      {text ? representation === 'body'
        ? <TrafficTextContent content={text} contentType={Object.entries(exchange.responseHeaders || {}).find(([name]) => name.toLowerCase() === 'content-type')?.[1]?.[0]} />
...
function ReplayDifference({ section, original, replayed, representation }: { section?: TrafficComparisonSection; original?: TrafficExchange; replayed?: TrafficExchange; representation: TrafficPayloadView }) {
  return <div className="replay-diff">
    ...
    <div className="replay-diff__panes">{original && <ReplayResponse exchange={original} representation={representation} label="Original" />}{replayed && <ReplayResponse exchange={replayed} representation={representation} label="Replayed" />}</div>
  </div>
}
