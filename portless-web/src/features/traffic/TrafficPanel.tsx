...
export function TrafficPanel({ environment }: { environment: Environment }) {
  ...
  useEffect(() => {
    let active = true
    let loading = false
    setTraces([]); setExchanges([]); setSelectedExchange(null); setSelectedTrace(null); setDetailNavigationPending(false); setTraceNavigationScoped(false); setSelectedTraceNavigationKey(null); setExpandedTrace(null); setError(null)
    selectionRequest.current += 1
    setTracePage(0); setExchangePage(0)
  ...
  const selectedTraceNavigationItems = useMemo(() => selectedTrace && traceNavigationScoped ? traceNavigationItems(selectedTrace, includeBackground) : undefined, [selectedTrace, traceNavigationScoped, includeBackground])
  ...
  const inspectVisibleExchange = (exchange: TrafficExchange) => {
    const index = visibleExchanges.findIndex((candidate) => candidate.sequence === exchange.sequence)
    if (index >= 0) setExchangePage(Math.floor(index / trafficPageSize))
  ...
  return <div className="traffic-view">
    ...
    <section className="panel traffic-panel">
      ...
      {mode === 'traces' ? <div className="trace-list">
        ...
        {tracePagination.items.map((trace) => <div className={`trace-card${expandedTrace === trace.number ? ' is-expanded' : ''}`} key={trace.number}>
          ...
          {expandedTrace === trace.number && (trace.spans?.length ? <TraceWaterfall trace={trace} includeBackground={includeBackground} onItem={(item) => inspectTraceItem(item, trace)} /> : <div className="trace-loading">Loading trace spans…</div>)}
        </div>)}
    ...
    {selectedExchange && (mode === 'exchanges'
      ? <ExchangeTraceDrawer
