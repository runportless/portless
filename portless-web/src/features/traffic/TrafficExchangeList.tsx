...
export function TrafficExchangeList({ pagination, onInspect, onPage }: {
  ...
  return <div className="exchange-list">
    ...
    {pagination.items.map((exchange) => <button className="table-row traffic-row" key={exchange.sequence} onClick={() => onInspect(exchange)}><code>#{exchange.sequence}</code><span>{trafficStartedTime(exchange.startedAt)}</span><strong>{isWebSocketHandshake(exchange) ? 'WS' : exchange.protocol === 'tcp' ? exchange.tcp?.applicationProtocol?.toUpperCase() || 'TCP' : 'HTTP'}</strong><code className="truncate">{exchangeOperation(exchange)}</code><span>{exchange.source}<i className="edge-arrow">→</i>{exchange.target}</span><span className={trafficResultTone(exchange.error || exchange.tcp?.outcome === 'error' || exchange.tcp?.outcome === 'incomplete', exchange.status)}>{exchangeResult(exchange)}</span><span>{duration(exchange.durationMs)}</span><span>{exchange.replay && <b className="replay-chip" title={`Replay of ${exchange.replay.project}/${exchange.replay.environment} request #${exchange.replay.sequence}`}>REPLAY </b>}{exchange.fault ? <b className="fault-chip">▲ {exchange.fault}</b> : exchange.recording ? <b className="record-chip">● {exchange.recording}</b> : exchange.replay ? null : '—'}</span></button>)}
