...
export function MockRouteEditor({ routingContext, scenario, services, routeName, draft, dirty, busy, previewing, error, onDismissError, onChange, onCancel, onSave, onPreview }: {
  ...
  const [headersExpanded, setHeadersExpanded] = useState<boolean | null>(null)
  ...
  useEffect(() => {
    if (error?.message.toLowerCase().includes('query')) {
      setQueryExpanded(true)
      setConfigurationTab('request')
    } else if (error?.message.toLowerCase().includes('header')) {
      setHeadersExpanded(true)
      setConfigurationTab('response')
    }
  }, [error])
  useEffect(() => {
    if (preview.error?.message.toLowerCase().includes('query')) setQueryExpanded(true)
    if (preview.error?.message.toLowerCase().includes('header')) {
      setHeadersExpanded(true)
      setConfigurationTab('response')
    }
  }, [preview.error])
    ...
    {missing ? <div className="mock-workspace-missing"><strong>ROUTE NOT FOUND</strong><p>{routeName} is no longer part of this mock scenario.</p><button className="button" type="button" onClick={onCancel}>BACK TO ROUTES</button></div> : <>
      ...
      <div className="mock-route-form__scroll" role="tabpanel" id={`${configurationID}-panel-request`} aria-labelledby={`${configurationID}-tab-request`} hidden={configurationTab !== 'request'} tabIndex={0}>
        {configurationTab === 'request' && <>
          <section className="mock-route-form__section">
            <div className="mock-route-identity-grid">
              ...
              <label className="mock-route-identity"><span>SERVICE</span><select aria-label="SERVICE" value={draft.service} disabled={busy} onChange={(event) => change('service', event.target.value)}>{services.map((service) => <option key={service}>{service}</option>)}</select></label>
            </div>
          ...
          <section className="mock-route-form__section mock-route-form__section--entries">
            <MockNameValueSection rows={draft.query} label="Query parameters" open={queryOpen} onOpenChange={(open) => { focusNewPath.current = false; setQueryExpanded(open) }} disabled={busy} onChange={(query) => { setQueryExpanded(true); change('query', query) }} />
          </section>
        </>}
      </div>
      <div className="mock-route-form__scroll" role="tabpanel" id={`${configurationID}-panel-response`} aria-labelledby={`${configurationID}-tab-response`} hidden={configurationTab !== 'response'} tabIndex={0}>
        {configurationTab === 'response' && <>
          ...
          <section className="mock-route-form__section mock-route-form__section--entries">
            <MockNameValueSection rows={draft.headers} label="Response headers" open={headersOpen} onOpenChange={setHeadersExpanded} disabled={busy} onChange={(headers) => { setHeadersExpanded(true); change('headers', headers) }} />
          </section>
        </>}
      </div>
      </form>
