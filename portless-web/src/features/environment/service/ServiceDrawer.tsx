...
export function ServiceDrawer({ environment, service, onClose, onChanged, onNavigate }: {
  ...
  >
    {drawerTab === 'details' && <>
      ...
      <section className="drawer-section service-endpoints"><div className="eyebrow">ENDPOINTS</div>{mockScenario && <div className="service-mock-binding"><span>{service.mock?.unmatchedRequests === 'forward' ? 'Partial mock' : 'Mock scenario'}: <strong title={mockScenario}>{mockScenario}</strong></span><a href={mockScenarioPath} onClick={(event) => {
