...
func (c *Commands) waitDown(ctx context.Context, client *apiclient.Client, operation model.Operation, timeout time.Duration, suppressEvents bool) (model.Operation, error) {
...
func (c *Commands) downAll(ctx context.Context, client *apiclient.Client, options downOptions) error {
...
func (c *Commands) printDownAll(result downAllOutput) error {
	if c.JSONOutput {
		if err := command.WriteJSON(c.Out, result); err != nil {
			return err
		}
		if len(result.Failures) > 0 {
			message := fmt.Sprintf("down failed for %d of %d environments", len(result.Failures), result.Targets)
			return command.WriteErrorOutput(c.Err, "DOWN_INCOMPLETE", message)
		}
		return nil
	}
	if result.Targets == 0 {
