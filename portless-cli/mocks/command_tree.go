...
func (c *Commands) mockCommand() *cobra.Command {
	...
	set.Flags().StringArrayVar(&options.query, "query", nil, "required exact query value as name=value; empty value requires presence (repeatable)")
	set.Flags().StringArrayVar(&options.queryRegex, "query-regex", nil, "required query regex as name=pattern, matching the whole value (repeatable)")
