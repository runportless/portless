...
func (c *Commands) listProjects(ctx context.Context, limit int) error {
	...
	response.Projects = command.Truncate(response.Projects, limit)
	if c.JSONOutput {
...
func (c *Commands) showProject(ctx context.Context, requested string) error {
	if requested != "" && c.EnvironmentOverride != "" {
		return command.UsageError("an explicit project cannot be combined with --env")
	}
	client, _, err := c.Daemon.Connect(ctx)
	if err != nil {
		return err
	...
	for _, service := range project.Services {
		fmt.Fprintf(c.Out, "  %-20s %-10s %s\n", service.Name, service.Kind, projectServiceType(service))
	}
	...
	for _, connection := range project.Connections {
		fmt.Fprintf(c.Out, "  %-28s %-10s env %s\n", connection.Source+":"+connection.Target, connection.Protocol, command.EmptyAs(connection.Environment, "—"))
	}
	return nil
}

func projectServiceType(service model.ServiceDefinition) string {
