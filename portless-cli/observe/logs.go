...
func (c *Commands) logs(ctx context.Context, requestedService string, options logsOptions) error {
	if err := command.ValidLimit(options.limit, 10_000); err != nil {
		return err
	}
	if options.since < 0 {
		return command.UsageError("--since cannot be negative")
	}
	...
	for {
		...
		if !options.tail {
			if c.JSONOutput {
				return command.WriteJSON(c.Out, logsOutput{Project: environment.Project, Environment: environment.Name, Entries: response.Entries})
			}
		...
		for _, entry := range response.Entries {
			...
			seen[key] = struct{}{}
			if c.JSONOutput {
				if err := command.WriteJSONLine(c.Out, entry); err != nil {
					return err
				}
			} else {
...
func (c *Commands) printLogs(environment model.Environment, entries []model.LogEntry, singleService, timestamps bool) {
...
func (c *Commands) printLogEntry(entry model.LogEntry, includeService, timestamps bool) {
	prefix := ""
	if timestamps {
