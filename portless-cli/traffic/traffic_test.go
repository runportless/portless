...
func TestHTTPTrafficHumanOutputAttributesMockScenarioAndRoute(t *testing.T) {
	application, output, _ := newTestCommands(t)
	event := model.TrafficExchange{
		Sequence: 4, Protocol: model.ProtocolHTTP, Source: "checkout", Target: "inventory",
		TargetProvider: model.ProviderMock, MockScenario: "sold-out", MockRoute: "lookup",
	...
	for _, expected := range []string{"mock=sold-out/lookup", "Provider:", "mock", "Mock scenario:", "sold-out", "Mock route:", "lookup"} {
		if !strings.Contains(output.String(), expected) {
