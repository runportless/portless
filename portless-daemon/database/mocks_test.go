...
func TestMockScenariosPersistServiceRoutesAndCloneWithEnvironment(t *testing.T) {
	...
	updated, err := controlStore.PutMockRoute(ctx, "store", "local", "SOLD-OUT", model.MockRoute{Name: "lookup", Service: "inventory", Method: "get", Path: "/inventory/{sku}", Query: map[string]string{"warehouse": "central"}, Status: 409, Headers: map[string]string{"Content-Type": "application/json"}, Body: `{"available":false}`, DelayMS: 25, Enabled: true})
	if err != nil {
		t.Fatal(err)
	}
	if len(updated.Routes) != 1 || updated.Routes[0].Service != "inventory" || updated.Routes[0].Method != "GET" || updated.Routes[0].Query["warehouse"] != "central" || updated.Routes[0].ModifiedAt.IsZero() {
