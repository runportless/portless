...
func (s *Service) decorateEnvironment(environment model.Environment) model.Environment {
	...
	allocations, _ := s.database.NetworkAllocations(context.Background(), scope)
...
func (s *Service) connectionBinding(target model.ServiceDefinition, connection model.Connection, host string, port int, targetEnvironment map[string]string, active bool) (providers.BindingResult, error) {
	if connection.Environment == "" {
		return providers.BindingResult{}, nil
	}
	if connection.Binding != "" {
		return s.resources.Bind(target, connection, providers.BindingContext{
			Environment: connection.Environment, Host: host, Port: port,
			TargetEnvironment: targetEnvironment, Active: active,
		})
	}
	if !active {
		return providers.BindingResult{SafeValues: map[string]string{connection.Environment: "not active"}}, nil
	}
	...
	return providers.BindingResult{
