...
func (s *Service) beginServiceModeChange(ctx context.Context, projectName, environmentName, serviceName, actor string, targetMode model.LaunchMode) (model.Operation, error) {
	...
	go func() {
		...
		latest, modeErr := s.store.Environment(background, projectName, environmentName)
		if modeErr != nil {
		...
		if s.processes.IsRunning(scope, serviceName) {
			_ = s.store.SetServiceStatus(background, scope, serviceName, model.ServiceStopping, "")
			...
			if modeErr = s.processes.Stop(background, scope, serviceName, 10*time.Second); modeErr != nil {
				_ = s.store.SetServiceStatus(background, scope, serviceName, model.ServiceFailed, modeErr.Error())
				s.failOperation(scope, operation, modeErr)
				return
			}
		}
		s.proxy.RemoveTarget(scope, serviceName)
		_ = s.store.SetServiceStatus(background, scope, serviceName, model.ServiceStarting, "")
		s.reconcileEnvironmentStatus(background, scope)
		...
		if currentEnvironment, lookupErr := s.store.Environment(background, projectName, environmentName); lookupErr == nil {
