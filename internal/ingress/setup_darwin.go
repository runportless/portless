...
func installPlatform(ctx context.Context, request SetupRequest) (resultErr error) {
	...
	if err != nil {
		return fmt.Errorf("inspect ingress receipt before provisioning loopback addresses: %w", err)
	}
	ownsLoopbackPool := false
	if receiptExists {
		receipt, receiptErr := readInstallationReceipt(currentPlatformInstallation())
		if receiptErr != nil {
			return fmt.Errorf("inspect ingress receipt before provisioning loopback addresses: %w", receiptErr)
		}
...
func waitForLaunchdUnloaded(ctx context.Context, timeout time.Duration, probe func(context.Context) (bool, error)) error {
	...
	for {
		...
		if !time.Now().Before(deadline) {
			return fmt.Errorf("ingress launch daemon %s is still loaded after %s", launchdLabel, timeout)
		}
		select {
		case <-ctx.Done():
			return ctx.Err()
