...
type Detection struct {
	...
	Explanation         string
	Markers             []string
	DefaultEnvironment  func(resource.Consumer) string
	ExplicitEnvironment func(string, resource.Consumer) string
}

func Detect(ctx context.Context, workspace resource.Workspace, consumers []resource.Consumer, config Detection) (resource.Findings, error) {
	var evidence []model.Evidence
	type detectedBinding struct {
		claim    resource.BindingClaim
		explicit bool
	}
	bindings := make(map[string]detectedBinding)
	for _, file := range workspace.Files() {
		if err := ctx.Err(); err != nil {
			return resource.Findings{}, err
		}
		...
		claim := resource.BindingClaim{ConsumerKey: consumer.Key, Environment: environment, Required: true}
		if existing, exists := bindings[consumer.Key]; exists {
			...
			if existing.explicit && explicit || !existing.explicit && !explicit {
				return resource.Findings{}, fmt.Errorf("resource configuration for %s uses conflicting environment variables %s and %s", consumer.Name, existing.claim.Environment, environment)
			}
	...
	if len(evidence) == 0 {
		return resource.Findings{}, nil
	}
	sort.Slice(evidence, func(i, j int) bool { return evidence[i].File < evidence[j].File })
	claims := make([]resource.BindingClaim, 0, len(bindings))
	...
	return resource.Findings{Candidates: []resource.Candidate{{Key: ".", Name: config.Name, Evidence: evidence, Bindings: claims}}}, nil
}
...
func OwningConsumer(file string, consumers []resource.Consumer) *resource.Consumer {
	directory := path.Dir(file)
	best := -1
	var owner *resource.Consumer
...
func FrameworkEnvironment(consumer resource.Consumer, spring, fallback string) string {
