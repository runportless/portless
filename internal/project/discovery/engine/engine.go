...
func New(config Config, detectors []spec.ServiceDetector, analyzers []spec.TopologyAnalyzer) (*Engine, error) {
	...
	if config.Resources == nil {
		var err error
		config.Resources, err = resource.NewRegistry()
		if err != nil {
			return nil, err
		}
	}
...
func (e *Engine) Discover(ctx context.Context, start string) (Result, error) {
	...
	consumers := make([]resource.Consumer, 0, len(resolved))
	consumerByKey := make(map[string]resource.Consumer, len(resolved))
	for _, service := range resolved {
...
func (e *Engine) normalizeResourceCandidate(workspace spec.Workspace, resourceType string, candidate resource.Candidate, consumers map[string]resource.Consumer) (string, model.ServiceDefinition, []model.Connection, bool, error) {
	key, ok := spec.CleanRelative(candidate.Key)
...
func Validate(definition model.ProjectModel, resources *resource.Registry) error {
	if resources == nil {
		var err error
		resources, err = resource.NewRegistry()
		if err != nil {
			return err
		}
	}
	...
	for _, connection := range definition.Connections {
		...
		if connection.Source != "external" && connection.Environment != "" {
			...
			if target.Kind == model.ServiceResource {
				preview, err := resources.Bind(target, connection, resource.BindingContext{Environment: connection.Environment, Active: false})
				if err != nil {
