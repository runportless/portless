...
func (fixtureResourcePlugin) Descriptor() resource.Descriptor {
	return resource.Descriptor{ID: "fixture-broker", DefaultVersion: "1"}
}

func (fixtureResourcePlugin) Detect(_ context.Context, _ resource.Workspace, consumers []resource.Consumer) (resource.Findings, error) {
	if len(consumers) == 0 {
		return resource.Findings{}, nil
	}
	consumer := consumers[0]
	return resource.Findings{Candidates: []resource.Candidate{{
		...
		Bindings: []resource.BindingClaim{{ConsumerKey: consumer.Key, Environment: "BROKER_URL", Required: true}},
	}}}, nil
}

func (fixtureResourcePlugin) Plan(definition model.ResourceDefinition) (resource.ContainerPlan, error) {
	return resource.ContainerPlan{
		Image: "docker.io/example/fixture-broker:" + definition.Version, ClientPort: 7777,
		Volumes:   []resource.Volume{{Key: "data", Path: "/data"}, {Key: "archive", Path: "/archive"}},
		Readiness: resource.Readiness{Kind: "tcp", Timeout: time.Minute, Interval: time.Second},
	}, nil
}

func (fixtureResourcePlugin) Bind(context resource.BindingContext) (resource.BindingResult, error) {
	if !context.Active {
...
func TestCustomResourcePluginNeedsNoEngineSpecialCase(t *testing.T) {
	...
	resources, err := resource.NewRegistry(fixtureResourcePlugin{})
	if err != nil {
		t.Fatal(err)
	}
