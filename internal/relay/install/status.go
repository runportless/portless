...
func Inspect(ctx context.Context) (InstallationStatus, error) {
	...
	status := InstallationStatus{
		...
		ResolverPath: details.ResolverPath, DNSListenAddress: relay.DefaultDNSAddress,
	}
	...
	if status.Installed {
		httpErr := relay.Check(ctx)
		dnsErr := relay.CheckDNS(ctx)
		resolverContext, cancelResolver := context.WithTimeout(ctx, 1500*time.Millisecond)
		resolverErr := relay.CheckResolver(resolverContext)
		cancelResolver()
		if httpErr != nil {
