...
func relayUninstallBlockers(status relayinstall.InstallationStatus, paths installation.Layout, uid int) []string {
	return relayUninstallBlockersWithValidator(status, paths, uid, relayinstall.ValidateOwnership)
}

func relayUninstallBlockersWithValidator(status relayinstall.InstallationStatus, paths installation.Layout, uid int, validateOwner func(relayinstall.InstallationStatus, int) error) []string {
	if !status.Installed {
		return nil
	}
...
func (c *CLI) removeRelayForUninstall(ctx context.Context, relay uninstallRelayOutput) (bool, error) {
	...
	uid, _ := c.local.userIDs()
	return c.local.uninstallRelay(ctx, relayinstall.UninstallRequest{
