...
func (m *Manager) inspectDaemon(ctx context.Context) (Inspection, error) {
	paths := m.layout
	record, err := instance.Read(paths)
	if err != nil {
...
// modifying it.
func (m *Manager) checkDaemon(ctx context.Context) (instance.Record, error) {
	inspection, err := m.inspectDaemon(ctx)
	if err != nil {
		return instance.Record{}, err
	}
	if !inspection.Compatible || !inspection.CurrentBuild {
		return instance.Record{}, incompatibleDaemonError(inspection)
	}
...
func unverifiedDaemonError(record instance.Record, cause error) error {
