...
func (m *Manager) resetApplicationState(ctx context.Context, force bool) (installation.ResetStateResult, instance.Record, error) {
	...
	if err := installation.EnsurePrivateDirectory(paths.Root); err != nil {
		return installation.ResetStateResult{}, instance.Record{}, err
	}
	...
	if inspectErr == nil {
		if len(inspection.Identity.ActiveEnvironments) > 0 && !force {
			return installation.ResetStateResult{}, instance.Record{}, &ActiveEnvironmentsError{Environments: append([]string(nil), inspection.Identity.ActiveEnvironments...)}
		}
	...
	if err != nil {
		return result, instance.Record{}, err
	}
	if err := m.hooks.StartDaemon(paths); err != nil {
		return result, instance.Record{}, fmt.Errorf("application state was erased, but the daemon could not restart: %w", err)
	}
	record, err := m.waitForResetDaemon(ctx)
	if err != nil {
		return result, instance.Record{}, fmt.Errorf("application state was erased, but the replacement daemon is not ready: %w", err)
	}
...
func (m *Manager) prepareStoppedDaemonForReset(ctx context.Context, inspectErr error) error {
	paths := m.layout
	record, recordErr := instance.Read(paths)
	...
	instance.RemoveMatching(paths, record)
	return verifyDaemonInstanceStopped(paths)
}
...
func (m *Manager) waitForResetDaemon(ctx context.Context) (instance.Record, error) {
	paths := m.layout
	...
	return instance.Record{}, errors.New(message)
}
