...
func (m *Manager) waitForInstanceStop(ctx context.Context, expected instance.Record, timeout time.Duration) error {
	paths := m.layout
	deadline := m.hooks.Now().Add(timeout)
	for {
		current, err := instance.Read(paths)
		if errors.Is(err, os.ErrNotExist) {
			return nil
		}
		...
		if aliveErr == nil && !alive {
			instance.RemoveMatching(paths, expected)
			return nil
		}
...
func (m *Manager) signalAndWait(ctx context.Context, record instance.Record, signal syscall.Signal, timeout time.Duration) error {
...
func verifyRecordedDaemonProcess(ctx context.Context, paths installation.Layout, record instance.Record) error {
	current, err := instance.Read(paths)
	if err != nil {
