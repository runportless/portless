...
func (m *Manager) ensureDaemon(ctx context.Context) (instance.Record, error) {
	paths := m.layout
	...
	for {
		...
		if m.hooks.Now().After(lockDeadline) {
			return instance.Record{}, errors.New("timed out waiting for another Portless CLI to prepare the daemon")
		}
	...
	if inspectErr == nil {
		...
		case recordErr == nil:
			...
			if aliveErr != nil {
				return instance.Record{}, fmt.Errorf("inspect recorded daemon process %d: %w", record.PID, aliveErr)
			}
			if alive {
				return instance.Record{}, unverifiedDaemonError(record, inspectErr)
			}
			instance.RemoveMatching(paths, record)
		case errors.Is(recordErr, os.ErrNotExist):
			// There is no daemon to replace.
		default:
			return instance.Record{}, fmt.Errorf("the daemon control record is invalid; refusing to start a second daemon: %w", recordErr)
		}
	}
