...
func TestForcedStopUsesInjectedProcessOperations(t *testing.T) {
	...
	record := instance.Record{PID: 4242, Port: 7331, APIVersion: "1.0.0", TokenPath: layout.AuthToken, InstanceID: "fixture"}
	if err := instance.Write(layout, record); err != nil {
		t.Fatal(err)
	}
	...
	manager := NewWithHooks(layout, Hooks{
		...
		VerifyProcess: func(_ context.Context, actual installation.Layout, candidate instance.Record) error {
	...
	if _, err := instance.Read(layout); !errors.Is(err, os.ErrNotExist) {
